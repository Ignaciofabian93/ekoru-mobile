...
export default function DatePicker({
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable onPress={handlePress} style={({ pressed }) => [styles.field, pressed && styles.fieldActive]}>
        <Calendar size={18} color={colors.foregroundTertiary} strokeWidth={2} />
        <Text style={[styles.value, !value && styles.placeholder]}>{displayValue || placeholder}</Text>
      </Pressable>
      ...
      {Platform.OS === "ios" && (
        <Modal isOpen={showPicker} onClose={() => setShowPicker(false)} title={label} size="sm">
          <DateTimePicker
            value={tempDate}
            mode="date"
            display="spinner"
          ...
          />
          <MainButton text={confirmLabel} onPress={handleIOSConfirm} fullWidth style={styles.confirmButton} />
        </Modal>
      )}
    </View>
  );
